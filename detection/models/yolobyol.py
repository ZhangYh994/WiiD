...
class BaseModelByol(nn.Module):
    ...
    def _forward_once(self, x, cons=None, profile=False, visualize=False, use_mmd=False):
        y, y_k, dt = [], [], []  # outputs
        # first sample
        for m in self.model:
        ...
        if cons is not None:
            ...
            for m in self.encoder_k:
                if m.f != -1:  # if not from previous layer
                    cons = y_k[m.f] if isinstance(m.f, int) else [cons if j == -1 else y_k[j] for j in m.f]  # from earlier layers
                if profile:
                    self._profile_one_layer(m, cons, dt)
                cons = m(cons)  # run
                y_k.append(cons if m.i in self.save else None)  # save output
                if visualize:
                    feature_visualization(cons, m.type, m.i, save_dir=visualize)

            # BYOL feature
            l1, l2, l3 = y[self.save[0]], y[self.save[1]], y[self.save[2]]
            ...
            if use_mmd:
                return x, feat_q, feat_k, y[self.save[-3]], y_k[self.save[-3]]
            return x, feat_q, feat_k
        return x
...
class DetectionModelByol(BaseModelByol):
    ...
    def __init__(self, cfg="yolov5s.yaml", ch=3, nc=None, anchors=None, m=0.99):  # model, input channels, number of classes
        super().__init__()
    ...
    def forward(self, x, cons=None, augment=False, profile=False, visualize=False, use_mmd=False):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        return self._forward_once(x, cons, profile, visualize, use_mmd)  # single-scale inference, train
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    no = na * (nc + 5)  # number of o

    # utputs = anchors * (classes + 5)
